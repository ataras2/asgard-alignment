...
# Tip/tilt 140 vector over full DM 
#TT = dmbases.get_tip_tilt_vectors()
TT = [np.nan_to_num(util.get_DM_command_in_2D( tt ),0) for tt in dmbases.get_tip_tilt_vectors().T]
...
# current_chn_data = dm_shm_dict[beam_id].shms[1].get_data()

# dm_shm_dict[beam_id].shms[1].set_data(  2*TT[0] + current_chn_data )

# dm_shm_dict[beam_id].shms[1].set_data(  current_chn_data )
...
for beam_id in args.beam_id:
    ...
    best_amp[beam_id] = amps[ np.argmax( slist ) ] #0.02 beam 2 seems good 

    dm_shm_dict[beam_id].set_data( best_amp[beam_id] * zbasis[3] )
