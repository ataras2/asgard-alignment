...
DM_serial_number_dict = {'1':'17DW019#113', '2': '17DW019#053', '3': '17DW019#122', '4':'17DW019#122'}
...
def apply_oscillating_mode(beam,  basis_name,  mode, speed, strength, duration, plot_shape=False, DMshapes_path = 'DMShapes/',SIMULATION = True):
    ...
    while (time.time() - start_time) < duration:
        ...
        if plot_shape:
            ...
            # Plot the DM command
            im = ax.imshow(get_DM_command_in_2D(dm_command), vmax=0.5 + np.max(strength * selected_mode), vmin=0.5 - np.max( strength * selected_mode) )
...
basis = st.selectbox("Select Basis", options=['Hadamard', 'Zonal', 'Zonal_pinned_edges', 'Zernike', 'Zernike_pinned_edges', 'fourier', 'fourier_pinned_edges'])
...
duration = st.number_input("Duration (seconds)", min_value=1, max_value=60, value=10, step=1)
plot_shape = st.checkbox("Plot Shape",value=True)

if st.button("Oscillate Mode"):
    apply_oscillating_mode(beam, basis, mode, speed, strength, duration, plot_shape, SIMULATION=simulation)