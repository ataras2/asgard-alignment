...
def apply_oscillating_mode(
    ...
    # No ! This is up the top of the file as a global.
    ...
    flatdm = pd.read_csv(
        DMshapes_path + "{}_FLAT_MAP_COMMANDS.csv".format(DM_serial_number_dict[beam]),
        header=None,
    )[0].values
    ...
    basis = construct_command_basis(
        basis=basis_name,
        number_of_modes=int(mode) + 10,
        Nx_act_DM=12,
        Nx_act_basis=12,
        act_offset=(0, 0),
        without_piston=True,
    ...
    if mode >= basis.shape[1] or mode < 0:
        print(
            f"Mode '{mode}' out of range. Please choose an integer between 0 and {basis.shape[1]-1}."
        )
        sys.exit(1)
    ...
    print(
        f"Oscillating mode {mode} from basis '{basis_name}' at {speed} Hz with strength {strength} on beam {beam} for {duration} seconds."
    )
    ...
    # Initialize the plot figure and axis outside the loop
    fig, ax = plt.subplots()
    colorbar = None  # Initialize colorbar as None
    ...
    while (time.time() - start_time) < duration:
        ...
        dm_command = flatdm + strength * selected_mode * (
            1 - 2 * round((speed * current_time) % 1)
        )
        if not SIMULATION:
            dm.send_data(dm_command)

        if plot_shape:
            # Clear the current axis and plot the new DM command
            ax.clear()

            # Plot the DM command
            im = ax.imshow(
                get_DM_command_in_2D(dm_command),
                #    vmax=0.5 + np.max(strength * selected_mode),
                #    vmin=0.5 - np.max( strength * selected_mode),
                vmax=1.0 - 0.3,
                vmin=0.0 + 0.3,
            )
            ax.set_title(f"DM Command at t={current_time:.2f}s")
            ...
            # colorbar = fig.colorbar(im, ax=ax, label='DM command')

            # Update the plot in the placeholder
            plot_placeholder.pyplot(fig)
        time.sleep(
            1.0 / (speed * 10)
        )  # Control loop timing for smooth oscillation - designed for slow oscillations!
...
speed = st.slider("Speed (Hz)", min_value=0.1, max_value=10.0, value=1.0, step=0.1)
strength = st.slider("Strength", min_value=0.0, max_value=1.0, value=0.1, step=0.01)
duration = st.number_input(
    "Duration (seconds)", min_value=1, max_value=60, value=10, step=1
)
plot_shape = st.checkbox("Plot Shape", value=True)

if st.button("Oscillate Mode"):
    apply_oscillating_mode(
        beam, basis, mode, speed, strength, duration, plot_shape, SIMULATION=simulation
    )
