...
N0sum = np.sum( N0_dict[beam_id], axis= (1,2) )
I0sum = np.sum( I0_dict[beam_id], axis= (1,2) )

plt.figure() 
plt.hist( N0sum / np.sum( np.mean( N0_dict[beam_id], axis= 0 ) ) ,alpha=0.5,label='clear pupil')
plt.hist( I0sum / np.sum( np.mean( I0_dict[beam_id], axis= 0 ) ) ,alpha=0.5,label='zwfs pupil')
plt.xlabel('(sum N0(t) / <sum N0>) in subframe')
plt.ylabel('frequency')
plt.legend()
plt.savefig('delme.png')

print( f"normalized clear pupil flux (sum N0(t) / <sum N0>) std = {np.std(  N0sum / np.sum( np.mean( N0_dict[beam_id], axis= 0 )  ))}" )
## ~ measure 0.2% std in normalized  flux sub in sub frame ok 
#  
...
sig = interpMatrix @ ((inn - I0n ) / N0n).reshape(-1,100)

reco = I2M.T @ sig

fig,ax = plt.subplots( len(reco), sharex=True, figsize=(5,12))
i=1
for rr,axx in zip( reco, ax.reshape(-1)):
    axx.hist( rr ,label=f'{i}')
    axx.legend()
    i+=1
...
close_after = 20
ki_grid = [0.2, 0.5, 0.8, 0.9, 0.97, 1, 1.2]
...
for ki_v in ki_grid:
    ...
    # save telemetry
    runn=f'TT_CL_static_100its_ki{ki_v}_v1'
    ...
    hdu = fits.ImageHDU(I2A[beam_id])
    hdu.header['EXTNAME'] = 'I2A'
    hdul.append(hdu)
...
# #### ILLUMINATED PUPIL DM 
# pup_on_dm = interpMatrix @ ( N0 - dark ).reshape(-1)

# plt.figure();plt.imshow( util.get_DM_command_in_2D( pup_on_dm) );plt.savefig('delme.png')

# # get outside actuators, edge actuators, secondary_actuators, central actuators 
