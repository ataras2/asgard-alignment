...
def interpolate_saturated_pixels(image, threshold):
    ...
    interpolated_values = griddata(valid_coords, valid_values, saturated_coords, method='cubic')
...
reduced_psf_meas  = interpolate_saturated_pixels( img_psf_region - np.mean( img_bkg ), threshold = 0.8)
...
nice_heatmap_subplots( im_list , xlabel_list, ylabel_list, title_list,cbar_label_list, fontsize=15, cbar_orientation = 'bottom', axis_off=True, savefig='/Users/bencb/Downloads/psf_beam3_strehl_measurement.png')


print( 'Strehl = ',np.max( reduced_psf_meas/np.sum(reduced_psf_meas) ) / np.max( airy_2D ) )
