...
dm_flat = zwfs.dm_shapes['BEST_calibrated_DMflat_12-09-2024T10.04.32']
...
if method=='double_sided':
    ...
    # the IM zero point 
    I0_IM = np.mean( zwfs.get_some_frames(number_of_frames = 256, apply_manual_reduction = True ) ,axis = 0 )
...
for Smax in Smax_grid: 
    ...
    for _ in range(500):
        ...
        disturbance_cmd = disturb_list[0]  # no disturbance
...
for Smax in Smax_grid: 
    ...
    for _ in range(500):
        ...
        disturbance_cmd = disturb_list[0]
...
for m in [43,55,65,75]:
    ...
    for Smax in Smax_grid: 
        ...
        #R  = (Vt.T * [1/ss if i < Smax else 0 for i,ss in enumerate(S)])  @ U.T
        disturbance_cmd = disturb_list[0] # static
        R_map = (phase_cov @ IM.T @ np.linalg.pinv(IM @ phase_cov.T @ IM.T + noise_cov) )
...
for m in [43,55,65,75]:
    ...
    zwfs.dm.send_data( dm_flat+ dm_pupil_filt * disturbance_cmd )
