...
pupil_crop_region = [170, 230, 100, 175]#before adam adj [160,220, 110,185] # [204,268,125, 187] #[None, None, None, None] #[204 -50 ,268+50,125-50, 187+50] 
...
# get new reference 
I0_now, N0_now = util.get_reference_images(zwfs, phasemask, theta_degrees=11.8, number_of_frames=256, \
compass = True, compass_origin=None, savefig= fig_path + exper_path + f'I0_N0_{now_tmp}.png' )
...
for no_modes in [30,40]: #[3,5,10,20,30,40,50,60]:
    ...
    if 1:
        ...
        pid.reset() 
        leak.reset()
        pid.kp = 0 * np.zeros( I2M_TT.shape[0] )
        pid.ki = 0 * np.zeros( I2M_TT.shape[0] )
        leak.rho = 0 * np.zeros( I2M_HO.shape[0] )
        leak.kp = 0 * np.zeros( I2M_HO.shape[0] )
        ...
        plt.xlabel('iterations')
        #plt.savefig(fig_path + 'delme.png')
        plt.savefig(current_path + f'rmse_{explabel}.png')
        # write telemetry to file 
        ...
        ax[0].imshow( s_list[0] )
        ax[1].imshow( s_list[-1] )
        #plt.savefig(fig_path + 'delme.png')
        plt.savefig(current_path + f'initial_v_final_image_{explabel}.png')
        # write telemetry to file 
        ...
        savefig = current_path + 'delme.png' 
        util.nice_heatmap_subplots( im_list , xlabel_list, ylabel_list, title_list,cbar_label_list, \
                            fontsize=15, cbar_orientation = 'bottom', vlims=vlims, axis_off=True, savefig='final_positions_{explabel}.png')
