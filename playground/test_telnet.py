...
if __name__ == "__main__":
    PDU_IP = "192.168.100.11" 

    pdu = AtenEcoPDU(PDU_IP)

    if pdu.connect():
        print("\n--- Reading Outlet Status (Outlet 1, simple) ---")
        status = pdu.read_outlet_status(1, "simple")
        print(f"Outlet 1 Status: {status}")
        time.sleep(1)  # Small delay between commands

        print("\n--- Reading PDU Voltage (simple) ---")
        pdu_voltage = pdu.read_power_value(
            "dev", measurement="volt", return_string="simple"
        )
        print(f"PDU Voltage: {pdu_voltage}")
        time.sleep(1)

        print("\n--- Reading Bank 1 Current (format) ---")
        bank_current = pdu.read_power_value(
            "bnk", number=1, measurement="curr", return_string="format"
        )
        print(f"Bank 1 Current: {bank_current}")
        time.sleep(1)

        print("\n--- Reading Outlet 4 Power (simple) ---")
        outlet_power = pdu.read_power_value(
            "olt", number=4, measurement="pow", return_string="simple"
        )
        print(f"Outlet 4 Power: {outlet_power}")
        time.sleep(1)

        print("\n--- Reading Environmental Sensor 1 (format) ---")
        sensor_value = pdu.read_environmental_value(1, "format")
        print(f"Sensor 1 Value: {sensor_value}")
        time.sleep(1)

        pdu.close()
    else:
        print("Failed to connect to PDU. Exiting.")
