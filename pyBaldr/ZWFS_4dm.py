...
class ZWFS():
    ...
    def __init__(self, DM_serial_number, cameraIndex=0, DMshapes_path='DMShapes/', pupil_crop_region=[None, None, None, None] ):
        ...
        dm = {}
        dm_err_flag = {}
        for beam, DM_serial_number in DM_serial_number.items():
            dm[beam] = bmc.BmcDm() # init DM object
            dm_err_flag[beam]  = dm[beam].open_dm(DM_serial_number) # open DM
            if dm_err_flag:
        ...
        if os.path.isdir(DMshapes_path):
            ...
            if filenames == []:
                ...
                for file in filenames:
                    for beam, serial_number in DM_serial_number.items():
                        if serial_number in file:
                            shape_name = file.split('/')[-1].split('.csv')[0]
                            shape = pd.read_csv(file, header=None)[0].values
                            if len(shape)==self.dm_number_of_actuators:
                                shapes_dict[f"flat_dm_{beam}"] = np.array( (shape) )
                            else: 
                                print(f'file: {file}\n has {len(shape)} values which does not match the number of actuators on the DM ({self.dm_number_of_actuators})')
        ...
        #THERE NEED TO BE UPDATED WITH EACH DM
        # reference centers 
    ...
    def send_cmd(self, cmd, beam):
       
        self.dm_err_flag[beam] = self.dm[beam].send_data(cmd)
    ...
    def exit_dm(self, beam):
        self.dm[beam].close_dm()
