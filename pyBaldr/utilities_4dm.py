...
def shape_dm_manually(zwfs, beams, compass = True , initial_cmd = None ,number_of_frames=5, apply_manual_reduction=True, theta_degrees=11.8, flip_dm=True, savefig=None):
    if initial_cmd == None:
        for b in beams:
            cmd =  zwfs.dm_shapes[f'flat_dm_{b}'].copy()
    else:
        assert len(cmd) == 140
        assert np.max(cmd) < 1
        assert np.min(cmd) > 0
...
def apply_sequence_to_DM_and_record_images(zwfs, beams, DM_command_sequence, number_images_recorded_per_cmd = 5, take_mean_of_images=False, save_dm_cmds = True, calibration_dict=None, additional_header_labels=None, sleeptime_between_commands=0.01, cropping_corners=None, save_fits = None):
    """
...
def GET_BDR_RECON_DATA_INTERNAL(zwfs, beams, number_amp_samples = 18, amp_max = 0.2, number_images_recorded_per_cmd = 10, source_selector = None,save_fits = None) :
    """
    source_selector is motor to move light source for bias frame, if None we ask to manually move it
    """


    modal_basis = np.eye(len(140))
    ...
    for b in beams:
        zwfs.dm[b].send_data( zwfs.dm_shapes[f'flat_dm_{b}']) 
    ...
    #### ## THIS NEEDS RE_WORKING ---- DO WE APPLY DMS ALL AT ONCE? 
    # ======== RAMPING ACTUATORS  
